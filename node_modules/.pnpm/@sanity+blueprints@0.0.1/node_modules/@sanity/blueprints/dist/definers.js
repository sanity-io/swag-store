...
const EVENT_KEYS = new Set(['on', 'filter', 'projection']);
export function defineDocumentFunction(functionConfig) {
    ...
    if (event) {
        ...
        const invalidKeys = Object.keys(maybeEvent).filter((k) => EVENT_KEYS.has(k));
        if (invalidKeys.length > 0) {
            throw new Error(`\`event\` cannot be specified with ${invalidKeys.map((k) => `\`${k}\``).join(', ')}`);
        }
        if (!event.on)
            throw new Error('`event.on` is required');
        if (!Array.isArray(event.on))
            throw new Error('`event.on` must be an array');
    }
    else {
        const { on, filter, projection } = maybeEvent;
        if (on && !Array.isArray(on))
            throw new Error('`on` must be an array');
