  ...
  try {
    ...
    if (!store?.tags) {
      ...
      if (Array.isArray(store.tags)) {
        ...
        tags = store.tags.split(',').map(tag => tag.trim()).filter(tag => tag.length > 0)
      } else {
      ...
      if (tags.length > 0) {
        ...
        for (const tag of parentTags) {
          ...
          console.log('🔄 Processing parent tag:', tag, '-> ProductMap:', productMapName, 'ID:', productMapId)
          ...
          if (existingProductMap) {
            ...
            if (!productAlreadyExists) {
              ...
              console.log('ℹ️ Product already exists in productMap:', productMapName, '- skipping duplicate')
            }
          } else {
            ...
            try {
              ...
              if (!verifyProductMap) {
                console.error('❌ Failed to create productMap:', productMapName, 'ID:', productMapId)
                productMapRef = null
              }
            } catch (error) {
        ...
        for (const tag of colorTags) {
          ...
          console.log('🔄 Processing color tag:', tag, '-> Color:', colorName, 'ID:', colorVariantId)
